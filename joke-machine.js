...
async function main() {
    if (options.list && !options.get && !options.rand) {
        ...
        if (!(await jokes.get(options.get))) {
            console.log(
                chalk.redBright("Invalid Category: ") +
                    chalk.whiteBright(options.get)
            );
        }
        ...
        if (
            (options.list == true && (options.get || options.rand == true)) ||
            (options.get && options.rand == true)
        ) {
            console.log(
                chalk.redBright("Argument Error: ") +
                    chalk.whiteBright(
                        "Can not Combine these Operations (joke-machine --help)"
                    )
            );
        }
    }
